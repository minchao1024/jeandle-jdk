...
define private hotspotcc void @jeandle.g1_pre_barrier(ptr addrspace(1) %addr) noinline "lower-phase"="1" #0 {
entry:
...
define private hotspotcc void @jeandle.g1_pre_barrier_loaded(ptr addrspace(1) %pre_val) noinline "lower-phase"="1" #0 {
entry:
  %marking_offset = load i32, ptr @G1ThreadLocalData.satb_mark_queue_active_offset
  %marking_adr = inttoptr i32 %marking_offset to ptr addrspace(2)
  %marking = load i8, ptr addrspace(2) %marking_adr
...
define private hotspotcc void @jeandle.g1_post_barrier(ptr addrspace(1) %addr, ptr addrspace(1) captures(none) %oop) noinline "lower-phase"="1" #0 {
entry:
