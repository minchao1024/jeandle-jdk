...
void JeandleAbstractInterpreter::fcmp(BasicType type, bool true_if_unordered) {
  ...
  // llvm::Value* cond = _ir_builder.CreateFCmpUNO(l, r);
  // llvm::Value* nan_result = is_g ? JeandleType::int_const(_ir_builder, 1) : JeandleType::int_const(_ir_builder, -1);
  // llvm::Value* zero = JeandleType::int_const(_ir_builder, 0);
  // llvm::Value* lt_cmp = _ir_builder.CreateFCmpULT(l, r);
  // llvm::Value* less_than = JeandleType::int_const(_ir_builder, -1);
  // llvm::Value* greater_than = JeandleType::int_const(_ir_builder, 1);

  // llvm::Value* if_not_nan = _ir_builder.CreateSelect(lt_cmp, less_than,
  //                                                    _ir_builder.CreateSelect(_ir_builder.CreateFCmpUEQ(l, r), zero, greater_than));
  // _jvm->ipush(_ir_builder.CreateSelect(cond, nan_result, if_not_nan));
  ...
  if (true_if_unordered) {
    ...
    non_negative_case = _ir_builder.CreateFCmpUGE(l, r);
  } else {
    negative_case     = _ir_builder.CreateFCmpULT(l, r);
    non_negative_case = _ir_builder.CreateFCmpOGE(l, r);
  }

  negative_case = _ir_builder.CreateZExt(negative_case, JeandleType::java2llvm(BasicType::T_INT, *_context));
