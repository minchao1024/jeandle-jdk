...
void JeandleAbstractInterpreter::fptosi_sat(llvm::Value* value, BasicType type) {
  llvm::Function* fptosi_sat = llvm::Intrinsic::getDeclaration(_module, Intrinsic::fptosi_sat,
                                                               {JeandleType::java2llvm(type, *_context), v->getType()});
  llvm::CallInst* call = _ir_builder.CreateCall(fptosi_sat, {v});
  _jvm->push(type, call);
}
